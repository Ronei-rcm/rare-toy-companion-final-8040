...
app.get('/api/customers/:userId/coupons', async (req, res) => {
  try {
    ...
    // Buscar cupons atribuídos ao usuário
    const [userCoupons] = await pool.execute(`
      ...
      AND c.active = TRUE
      ORDER BY 
        CASE WHEN uc.is_used = FALSE AND (uc.expires_at IS NULL OR uc.expires_at > NOW()) THEN 0 ELSE 1 END,
        uc.assigned_at DESC
    `, [userId]);
    ...
    const coupons = userCoupons.map(coupon => ({
      ...coupon,
      used: coupon.is_used,
      expiresAt: coupon.user_expires_at || coupon.expiresAt,
      isExpired: coupon.user_expires_at ? new Date(coupon.user_expires_at) < new Date() : (coupon.expiresAt ? new Date(coupon.expiresAt) < new Date() : false)
    }));
...
app.post('/api/customers/:userId/coupons/redeem', async (req, res) => {
  try {
    ...
    if (existing.length > 0) {
      return res.status(400).json({ error: 'Você já possui este cupom' });
    }
    ...
    // Atribuir cupom ao usuário
    const { v4: uuidv4 } = require('uuid');
    const userCouponId = uuidv4();
    await pool.execute(`
      INSERT INTO user_coupons (id, user_id, coupon_id, expires_at)
      VALUES (?, ?, ?, ?)
    `, [userCouponId, userId, coupon.id, coupon.expires_at]);
...
// Obter informações de fidelidade
app.get('/api/customers/:userId/loyalty', async (req, res) => {
  try {
    let { userId } = req.params;
    ...
    const [orders] = await pool.execute(`
      SELECT COUNT(*) as total, SUM(total_amount) as total_spent
      FROM orders
      WHERE user_id = ? AND status != 'cancelled'
    `, [userId]);

    const [couponsUsed] = await pool.execute(`
      SELECT COUNT(*) as count, COALESCE(SUM(discount_amount), 0) as total_savings
      FROM coupon_usage
      WHERE user_id = ?
    `, [userId]);
    ...
    const totalSavings = parseFloat(couponsUsed[0].total_savings || 0);
    const couponsRedeemed = couponsUsed[0].count || 0;

    // Calcular nível e pontos
    let level = 'Bronze';
    let points = Math.floor(totalSpent / 10); // 1 ponto a cada R$10
...
// Atribuir cupom automático (aniversário, primeira compra, etc)
app.post('/api/customers/:userId/coupons/auto-assign', async (req, res) => {
  try {
    let { userId } = req.params;
    const { category, reason } = req.body; // category: 'birthday', 'first_purchase', 'loyalty', etc
    ...
    // Buscar cupom da categoria especificada  
    // Nota: categoria não existe na tabela atual, usar fallback
    const [coupons] = await pool.execute(`
      SELECT * FROM coupons 
      WHERE active = TRUE
      AND expires_at >= NOW()
    ...
    if (coupons.length === 0) {
      return res.status(404).json({ error: 'Nenhum cupom disponível para esta categoria' });
    }
    ...
    if (existing.length > 0) {
      return res.json({ 
        success: true, 
        message: 'Cupom já atribuído anteriormente',
    ...
    await pool.execute(`
      INSERT INTO user_coupons (id, user_id, coupon_id, expires_at)
      VALUES (?, ?, ?, ?)
    `, [userCouponId, userId, coupon.id, expiresAt]);

    res.json({ 
      success: true, 
      message: `Cupom de ${reason || category} atribuído com sucesso!`,
      coupon: {
        id: coupon.id,
        code: coupon.code,
        title: coupon.title,
    ...
    console.error('Erro ao atribuir cupom automático:', error);
    res.status(500).json({ error: 'Erro ao atribuir cupom' });
  }
});
