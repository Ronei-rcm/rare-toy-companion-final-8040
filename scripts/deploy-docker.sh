...
# 🚀 DEPLOY COM MYSQL EM DOCKER - MuhlStore v1.0.7
...
echo -e "${BLUE}🐳 DEPLOY COM MYSQL EM DOCKER - MUHLSTORE v1.0.7${NC}"
...
# Iniciar MySQL em Docker
log "Iniciando MySQL em Docker..."
docker run -d \
...
for i in {1..10}; do
    ...
    warning "Aguardando MySQL... tentativa $i/10"
    sleep 3
done
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`🐳 MySQL rodando em Docker`);
...
case $opcao in
    ...
    8)
        ...
        ls -lh /home/espelhar/backup-*.sql | tail -5
        ;;
    9)
...
cat > /home/espelhar/monitor-muhlstore.sh << 'EOF'
#!/bin/bash
while true; do
    clear
    echo "🚀 MUHLSTORE - Monitor em Tempo Real"
    echo "======================================"
    echo ""
    echo "=== PM2 ===="
    pm2 list
    echo ""
    echo "=== Docker MySQL ===="
    docker stats muhlstore-mysql --no-stream 2>/dev/null || echo "MySQL não está rodando"
    echo ""
    echo "=== API Health ===="
    curl -s http://localhost:3001/api/health 2>/dev/null || echo "API não está respondendo"
    echo ""
    echo "======================================"
...
EOF
chmod +x /home/espelhar/monitor-muhlstore.sh
...
echo -e "${GREEN}  Monitor: /home/espelhar/monitor-muhlstore.sh${NC}"
echo ""
...
echo -e "${GREEN}  Parar: docker stop muhlstore-mysql${NC}"
echo -e "${GREEN}  Iniciar: docker start muhlstore-mysql${NC}"
echo ""
