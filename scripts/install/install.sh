...
# =============================================================================
# 🚀 MUHLSTORE - INSTALADOR AUTOMÁTICO COMPLETO v2.0
# =============================================================================
# Data: Outubro 2025
...
check_system() {
    ...
    if [[ $EUID -eq 0 ]]; then
        ...
        print_info "Execute como usuário normal com sudo habilitado"
        exit 1
    fi
...
install_nodejs() {
    ...
    # Detectar usuário real (não sudo)
    if [ -n "$SUDO_USER" ]; then
        REAL_USER=$SUDO_USER
    else
        REAL_USER=$(whoami)
    ...
    if [ ! -d "$REAL_HOME/.nvm" ]; then
        ...
        print_success "NVM instalado com sucesso"
    else
...
install_mysql() {
    ...
    print_info "Configurando MySQL (senha root será solicitada)"
...
EOF
    ...
    print_info "sudo mysql -u root -p < /tmp/mysql_setup.sql"
...
show_final_info() {
    ...
    print_info "   sudo systemctl status nginx - Status do Nginx"
    print_info "   sudo systemctl status mysql - Status do MySQL"
    ...
    print_info "   1. Configure o MySQL: sudo mysql -u root -p < /tmp/mysql_setup.sql"
