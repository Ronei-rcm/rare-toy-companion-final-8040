...
export function AdvancedProductsView() {
  ...
  // Carregar via server-side quando em Admin (sempre nesta view)
  useEffect(() => {
    const load = async () => {
      try {
        setServerLoading(true);
        const resp = await productsApi.getProductsPaged({
          page,
          pageSize,
          search: filters.search || undefined,
          categoria: filters.categories[0] || undefined,
          sort:
            filters.sortBy === 'name' ? 'nome_asc' :
            filters.sortBy === 'price_asc' ? 'preco_asc' :
            filters.sortBy === 'price_desc' ? 'preco_desc' : 'created_at_desc',
          inStock: filters.inStock || undefined,
          onSale: filters.onSale || undefined,
          featured: filters.featured || undefined,
          novo: filters.new || undefined,
        });
        setServerItems(resp.items || []);
        setServerTotal(resp.total || 0);
      } catch (e) {
        // fallback silencioso: mantém client-side
        setServerItems(null);
        setServerTotal(null);
      } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      ) : ((serverItems ?? filteredProducts) || []).length === 0 ? (
        <Card className="p-12">
      ...
      {(serverTotal ?? filteredProducts.length) > 0 && (
        <div className="flex items-center justify-between mt-6">
          <div className="text-sm text-gray-600">
            Mostrando {(page - 1) * pageSize + 1}–
            {Math.min(page * pageSize, serverTotal ?? filteredProducts.length)} de {serverTotal ?? filteredProducts.length}
          </div>
          <div className="flex items-center gap-3">
            <Select value={String(pageSize)} onValueChange={(v: any) => setPageSize(parseInt(v, 10))}>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                variant="outline"
                disabled={page * pageSize >= (serverTotal ?? filteredProducts.length)}
                onClick={() => setPage((p) => (p * pageSize >= (serverTotal ?? filteredProducts.length) ? p : p + 1))}
              >
                Próxima
              </Button>
            </div>
