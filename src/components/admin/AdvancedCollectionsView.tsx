...
export function AdvancedCollectionsView() {
  ...
  // Helper: contar produtos independente do formato retornado pelo backend
  const getProductsCount = (c: any): number => {
    if (!c) return 0;
    if (Array.isArray(c.produtos)) return c.produtos.length;
    if (typeof c.total_produtos !== 'undefined') return parseInt(c.total_produtos || 0);
    if (typeof c.produtos === 'number') return parseInt(c.produtos || 0);
    return 0;
  };
  ...
  const stats = useMemo(() => {
    ...
    const withProducts = collections.filter((c: any) => getProductsCount(c) > 0).length;
    const totalProducts = collections.reduce((sum: number, c: any) => sum + getProductsCount(c), 0);
  ...
  const filteredCollections = useMemo(() => {
    ...
    filtered.sort((a: any, b: any) => {
      switch (sortBy) {
        ...
        case 'products-desc':
          return getProductsCount(b) - getProductsCount(a);
        case 'date-desc':
  ...
  // Atualizar lista/estatísticas quando vínculos de produtos mudarem
  useEffect(() => {
    const handler = () => refetch?.();
    window.addEventListener('collectionUpdated', handler as any);
    return () => window.removeEventListener('collectionUpdated', handler as any);
  }, [refetch]);
                  ...
                  <div className={cn(
                    ...
                    <CardContent className="p-4 flex-1">
                      ...
                      <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
                        ...
                        <span>{getProductsCount(collection)} produto(s)</span>
                      </div>
                ...
                <Card className="bg-red-50 border-red-200 p-4">
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{getProductsCount(collectionToDelete)}</span>
                    </div>
                    <div className="flex justify-between">
