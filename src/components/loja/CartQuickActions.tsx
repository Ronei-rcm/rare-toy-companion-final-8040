...
const CartQuickActions: React.FC = () => {
  ...
  const handleCopyCart = () => {
    const cartText = state.itens
      .map((item) => `${item.produto.nome} (${item.quantidade}x) - R$ ${Number((item.produto.preco * item.quantidade) || 0).toFixed(2)}`)
      .join('\n');
    
    const fullText = `🛒 Meu Carrinho - MuhlStore\n\n${cartText}\n\nTotal: R$ ${Number(state.total || 0).toFixed(2)}`;
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Confira os produtos que escolhi na MuhlStore! Total: R$ ${Number(state.total || 0).toFixed(2)}`,
