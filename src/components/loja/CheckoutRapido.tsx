...
import { CreditCard, QrCode, User, MapPin, Phone, Mail, Loader2, CheckCircle, Zap, Smartphone, Wallet } from 'lucide-react';
...
const CheckoutRapido = ({ isOpen, onClose, variant = 'page' }: CheckoutRapidoProps) => {
  ...
  const validarDados = () => {
    ...
    const camposVazios = camposObrigatorios.filter(campo => !dadosCliente[campo as keyof typeof dadosCliente]);
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {step === 'dados' && (
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-lg">Cupom</CardTitle>
              </CardHeader>
              <CardContent className="flex gap-2">
                <Input value={cupom} onChange={(e) => setCupom(e.target.value)} placeholder="Ex: FRETEGRATIS" />
                <Button type="button" variant="outline" onClick={aplicarCupom}>Aplicar</Button>
              </CardContent>
            </Card>
          )}
          ...
          {step === 'dados' && (
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={finalizarPedido} className="flex-1" disabled={isSubmitting || !user}>
                  {isSubmitting ? (
                    <>
                      <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                      Processando...
