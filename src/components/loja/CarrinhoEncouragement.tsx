...
const CarrinhoEncouragement = () => {
  ...
  const getEncouragementMessage = () => {
    ...
    if (state.total === 0) {
      ...
      return {
        title: "🌟 Ótimo começo!",
        message: `Você já tem R$ ${Number(state.total || 0).toFixed(2)} no carrinho. Adicione mais R$ ${Number(valorRestanteFrete || 0).toFixed(2)} e ganhe frete grátis!`,
      ...
      return {
        title: "🚀 Quase lá!",
        message: `Faltam apenas R$ ${Number(valorRestanteFrete || 0).toFixed(2)} para ganhar frete grátis! Você está ${Number(((state.total / freeShippingMin) * 100) || 0).toFixed(0)}% do caminho!`,
        icon: <Truck className="h-6 w-6" />,
                  ...
                  <div key={p.id} className="rounded-md border bg-card hover:shadow-md transition-shadow">
                    ...
                    <div className="p-2 space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="text-xs text-primary font-semibold">R$ {Number(p.preco || 0).toFixed(2)}</div>
                        {p.promocao && (
