              ...
              <div className="rounded-md border p-3 bg-muted/40 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center rounded-full bg-green-100 text-green-800 px-2 py-0.5 border border-green-200">
                    Frete grátis acima de R$ {Number(settings.free_shipping_min || 0).toFixed(2)}
                  </span>
                        ...
                        <div className="flex items-center justify-between mt-2">
                          ...
                          <div className="text-right min-w-[80px]">
                            <div className="font-medium text-sm">
                              R$ {Number((produto.preco * item.quantidade) || 0).toFixed(2)}
                            </div>
            ...
            <DrawerFooter className="space-y-3 bg-muted/20">
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center text-sm">
                  <span>Subtotal</span>
                  <span>R$ {Number(state.total || 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span>Frete</span>
                  <span>{state.total > settings.free_shipping_min ? 'Grátis' : `R$ ${Number(settings.shipping_base_price || 0).toFixed(2)}`}</span>
                </div>
                <div className="flex justify-between items-center text-lg font-semibold">
                  <span>Total</span>
                  <span>
                    R$ {Number((state.total + (state.total > settings.free_shipping_min ? 0 : settings.shipping_base_price)) || 0).toFixed(2)}
                  </span>
                </div>
              </div>
              {!user && (
