...
const UsuariosAdmin = () => {
  ...
  const handleSave = async () => {
    ...
    if (showAddDialog) {
      // Capturar valor real dos inputs (cobre casos de autofill que não disparam onChange)
      const senhaEl = document.getElementById('senha') as HTMLInputElement | null;
      const confirmarEl = document.getElementById('confirmarSenha') as HTMLInputElement | null;
      const senhaValue = senhaEl?.value ?? formData.senha;
      const confirmarValue = confirmarEl?.value ?? formData.confirmarSenha;
      ...
      // Sincroniza estado se veio por autofill
    ...
    try {
      ...
      const payload = {
        ...
        // Em criação, sempre envie a senha (após validação acima); em edição, só se fornecida
        ...(showAddDialog
          ? { senha: (document.getElementById('senha') as HTMLInputElement | null)?.value || formData.senha }
