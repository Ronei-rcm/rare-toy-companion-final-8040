...
export function useSecurityEnhancement() {
  ...
  const getSecurityStatistics = useCallback(() => {
    ...
    const events24h = securityEvents.filter(e => e.timestamp >= last24h);
    const events7d = securityEvents.filter(e => e.timestamp >= last7d);
    const criticalAlerts = securityAlerts.filter(a => a.severity === 'critical' && a.status === 'new');
    const highAlerts = securityAlerts.filter(a => a.severity === 'high' && a.status === 'new');
    ...
    return {
      ...
      resolvedAlerts: securityAlerts.filter(a => a.status === 'resolved').length,
